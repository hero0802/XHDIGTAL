...
var leftPanel=Ext.create('Ext.form.Panel',{
	...
	items:[{
		...
		items:[{
			xtype:'fieldset',title:'类型',margin:'0 10 0 10',
			items:[{
				xtype:'radiogroup',fieldLabel:'',name:'ig',checked:true,labelWidth:10,layout:'form',margin:'0 0 10 0',
				items: [
			            { boxLabel: '用户', name:'ig', inputValue: '0',checked: true },
			            { boxLabel: '组', name:'ig', inputValue: '1'}
			        ],
			    listeners:{
				'change':function(){
				var form=leftPanel.getForm();
				if(form.findField('ig').getValue()['ig']==0){
					Ext.getCmp('user').setTitle('用户');
					userPanel.form.findField('gn').setValue(200);
					userPanel.form.findField('gnEnd').setValue(200);
				}else{
					Ext.getCmp('user').setTitle('组')
					userPanel.form.findField('gn').setValue(900);
					userPanel.form.findField('gnEnd').setValue(900);
				}
			}
			}
			}]
